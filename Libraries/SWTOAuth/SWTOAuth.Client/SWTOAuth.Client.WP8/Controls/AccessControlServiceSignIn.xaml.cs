...
namespace SWTOAuth.Client.WP8.Controls
{
    ...
    public partial class AccessControlServiceSignIn : UserControl
    {
        ...
        /// </summary>
        public static event EventHandler<RequestSecurityTokenResponseCompletedEventArgs> RequestSecurityTokenResponseCompleted;
        ...
        private async void IdentityProviderList_Refresh(Uri identityProviderDiscoveryService)
        {
            try
            {
                JSONIdentityProviderDiscoveryClient jsonClient = new JSONIdentityProviderDiscoveryClient();
                var providers = await jsonClient.GetIdentityProviderListAsync(identityProviderDiscoveryService);

                IdentityProviderList.ItemsSource = providers;
                // Preselect the only choice if it is available                
                if (providers.Count() == 1)
                {
                    HideAll();
                    IdentityProviderList.SelectedIndex = 0;
                }
                else
        ...
        { // navigate if at empty page, and a idp hrd url is available
            if (e.Uri.ToString() == "" && null != _selectedIdentityProvider)
            {
